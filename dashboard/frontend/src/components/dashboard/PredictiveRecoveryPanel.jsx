...
export default function PredictiveRecoveryPanel({ state, actions }) {
  ...
  return (
    <Panel title="Predictive Network Insights" className="predictive-recovery-panel">
      <ToggleField 
        label="Enable Predictive Network Insights"
      ...
      <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'var(--surface2)', borderRadius: 'var(--r-sm)' }}>
        <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-muted)' }}>
          {dashboard.predictiveRecoveryEnabled 
            ? `Predictive models are ONLINE. Real-time insights are calculating risks dynamically.`
            : 'Predictive analytics are OFFLINE. Network responds to failures reactively without telemetry foresight.'}
        </p>
      </div>
    </Panel>
  );
