...
export default function TopologySummary({ state }) {
  ...
  if (topology.status === 'running') statusBadgeType = 'ok';
  else if (topology.status === 'stopped') statusBadgeType = 'warning';
  else if (topology.status === 'launching') statusBadgeType = 'info';
    ...
    <Panel title="Topology Summary">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.5rem' }}>
          ...
          <Badge label={topology.status.toUpperCase()} type={statusBadgeType} />
        </div>
